...
jest.unstable_mockModule('@/utils/db', () => ({
	prisma: {
		...
		inboxAgentRegistrationRequest: {
			findFirst: mockFindInboxRegistration,
		},
	},
}));
...
const { queryInboxAgentByIdentifierGet } = await import('./index');
...
describe('queryInboxAgentByIdentifierGet', () => {
	beforeEach(() => {
		jest.clearAllMocks();
	...
	it('rejects wallet-scoped lookups for inbox agents not owned by the scoped holder wallet', async () => {
