...
export const postGenerateInvoiceSchemaInput = z
	.object({
		...
		blockchainIdentifier: z.string().describe('The blockchain identifier, for which the invoice should be created'),
...
export function checkIfCorrectionInvoice(
	...
	const existingDateFormatter = new Intl.DateTimeFormat(existingInvoice.localizationFormat, {
		dateStyle: 'short',
	});

	const formattedExistingDate = existingDateFormatter.format(existingInvoice.invoiceDate);
		...
		correctionInvoice: isCorrection
			? {
					...
					correctionDescription: resolved.texts.correctionDefault(
						generateInvoiceId(existingInvoice.revisionNumber, existingInvoice.InvoiceBase.invoiceId),
...
export const postGenerateInvoiceEndpoint = adminAuthenticatedEndpointFactory.build({
	method: 'post',
	...
	handler: async ({
		...
		try {
			...
			if (payment == null) {
				recordBusinessEndpointError('/api/v1/invoice', 'POST', 404, 'Payment not found', {
			...
			if (resolvePaymentKeyHash(input.walletAddress) !== resolvePaymentKeyHash(payment.BuyerWallet.walletAddress)) {
				throw createHttpError(400, 'Wallet is not the buyer wallet');
			}

			const decidedIdentifier = decodeBlockchainIdentifier(input.blockchainIdentifier);
			...
			const resolved = resolveInvoiceConfig(input.invoiceCurrency, input.invoice);
			...
			//
			const dateOfConversion = Math.min(Number(payment.payByTime ?? 1000000000000000000n), Number(payment.unlockTime));
			if (dateOfConversion == 0 || isNaN(dateOfConversion) || dateOfConversion > Date.now()) {
			...
			const createdInvoice = await prisma.$transaction(
				async (tx) => {
					...
					const correctionData = checkIfCorrectionInvoice(existingInvoice, groups, input.seller, input.buyer, resolved);
					if (!correctionData.isCorrectionInvoice && existingInvoice) {
						return {
							invoice: Buffer.from(existingInvoice.generatedPDFInvoice as unknown as Uint8Array).toString('base64'),
						};
					}
					...
					if (existingInvoice) {
						...
						newInvoiceId = generateInvoiceId(revisionNumber, existingInvoice.InvoiceBase.invoiceId);
					} else {
						revisionNumber = 0;
					...
					await tx.invoiceRevision.create({
						data: {
							...
							correctionInvoiceOriginalNumber: correctionData.correctionInvoice?.originalInvoiceNumber ?? null,
							correctionInvoiceOriginalDate: correctionData.correctionInvoice?.originalInvoiceDate ?? null,
							correctionInvoiceReason: correctionData.correctionInvoice?.correctionReason ?? null,
							correctionInvoiceTitle: correctionData.correctionInvoice?.correctionTitle ?? null,
							correctionInvoiceDescription: correctionData.correctionInvoice?.correctionDescription ?? null,
			...
			const statusCode =
				...
				500;
			recordBusinessEndpointError('/api/v1/signature', 'POST', statusCode, errorInstance, {
				user_id: options.id,
				wallet_address: input.walletAddress,
				operation: 'verify_signature',
