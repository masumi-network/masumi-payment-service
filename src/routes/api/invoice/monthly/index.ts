...
export const postGenerateMonthlyInvoiceSchemaInput = z
  .object({
    ...
    action: z
      .enum(['retrieve_monthly_invoices'])
      .describe('The action to perform for monthly invoices'),
    buyerWalletVkey: z
      .string()
      .min(1)
      .max(1000)
      .describe('The buyer wallet vkey to aggregate the month for'),
    month: z
      .string()
      .regex(/^\d{4}-\d{2}$/)
...
export const postGenerateMonthlyInvoiceEndpoint =
      ...
      try {
        ...
        if (payments.length === 0) {
          recordBusinessEndpointError(
            ...
            'POST',
            404,
            'No payments found for month and wallet',
            {
        ...
        if (
          ...
          resolvePaymentKeyHash(anyWalletAddress)
        ) {
          ...
          for (const missingConversion of missingConversionList) {
            for (const idData of idMapping) {
              ...
              if (missingConversion == '') {
                ...
                if (!cardanoPlatform) continue;
                decimals = cardanoPlatform.decimal_place as unknown as
                  | number
                  | null;
              }
        ...
        const dbItems: Array<
          InvoiceGroupItemInput & { referencedPaymentId: string }
        > = [];

        const groupedPayments = new Map<
          string,
          Array<(typeof payments)[number]>
        >();
