...
export const queryPaymentDiffSchemaInput = z.object({
	...
	cursorId: z
		.string()
		.optional()
		.describe('Pagination cursor (payment id). Used as tie-breaker when lastUpdate equals a payment change timestamp'),
...
function buildPaymentDiffOrderBy(mode: PaymentDiffMode): Prisma.PaymentRequestOrderByWithRelationInput[] {
	switch (mode) {
		case 'nextActionLastChangedAt':
