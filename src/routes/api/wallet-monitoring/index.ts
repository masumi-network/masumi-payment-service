...
const assetThresholdSchema = z.object({
	...
	displayName: z.string().nullable().describe('Human-readable name of the asset'),
	displaySymbol: z.string().nullable().describe('Display symbol for the asset (e.g., USDM)'),
	decimals: z.number().describe('Number of decimal places for this asset'),
	minAmount: z.string().describe('Minimum amount threshold (as string for large numbers)'),
});
...
const walletThresholdSchema = z.object({
	...
	enabled: z.boolean().describe('Whether monitoring is enabled for this wallet'),
	adaThresholdLovelace: z.string().describe('ADA threshold in lovelace (1 ADA = 1,000,000 lovelace)'),
	HotWallet: z.object({
	...
	AssetThresholds: z.array(assetThresholdSchema).describe('Thresholds for other assets (USDM, etc.)'),
});
...
export const walletMonitorConfigSchema = z
	.object({
		...
		paymentSourceId: z.string().describe('Payment source this monitoring config belongs to'),
		enabled: z.boolean().describe('Whether monitoring is enabled for this payment source'),
		checkIntervalSeconds: z.number().describe('How often to check balances (in seconds)'),
		lastCheckedAt: z.date().nullable().describe('Last time balances were checked'),
		lastCheckStatus: z.string().nullable().describe('Status of last check (success, partial_failure, error)'),
		lastCheckError: z.string().nullable().describe('Error message from last check, if any'),
		WalletThresholds: z.array(walletThresholdSchema).describe('Individual wallet thresholds'),
	})
...
export const getWalletMonitoringInputSchema = z.object({
	paymentSourceId: z.string().optional().describe('Filter by specific payment source ID'),
	network: z.nativeEnum(Network).optional().describe('Filter by network (Preprod or Mainnet)'),
});

export const getWalletMonitoringOutputSchema = z.object({
	WalletMonitorConfigs: z.array(walletMonitorConfigSchema).describe('List of wallet monitoring configurations'),
});
...
export const walletMonitoringGet = adminAuthenticatedEndpointFactory.build({
	...
	handler: async ({ input, ctx }: { input: z.infer<typeof getWalletMonitoringInputSchema>; ctx: AuthContext }) => {
		const configs = await prisma.walletMonitorConfig.findMany({
			where: {
				paymentSourceId: input.paymentSourceId,
				PaymentSource: {
					network: input.network ? { equals: input.network } : { in: ctx.networkLimit },
					deletedAt: null,
				},
			},
			include: {
...
export const createWalletMonitoringInputSchema = z.object({
	paymentSourceId: z.string().describe('Payment source ID to add monitoring to'),
	enabled: z.boolean().default(false).describe('Whether monitoring is enabled (default: false for safety)'),
	checkIntervalSeconds: z.coerce
		.number()
		.min(60)
		.max(86400)
		.default(3600)
		.describe('How often to check balances in seconds (min: 60, max: 86400, default: 3600 = 1 hour)'),
	walletThresholds: z
		.array(
			z.object({
				hotWalletId: z.string().describe('Hot wallet ID to monitor'),
				enabled: z.boolean().default(true).describe('Whether this wallet threshold is enabled'),
				...
				assetThresholds: z
					.array(
						z.object({
							policyId: z.string().length(56).describe('Policy ID of the asset (56 hex characters)'),
							assetName: z.string().describe('Asset name in hex (can be empty string)'),
							displayName: z.string().optional().describe('Human-readable name (e.g., "USD Masumi")'),
							displaySymbol: z.string().optional().describe('Display symbol (e.g., "USDM")'),
							decimals: z.coerce
								.number()
								.min(0)
								.max(18)
								.default(0)
								.describe('Number of decimal places (0 for no decimals, 6 for USDM)'),
							minAmount: z.string().max(25).describe('Minimum amount threshold in smallest unit'),
						}),
					)
					.optional()
...
export const walletMonitoringPost = adminAuthenticatedEndpointFactory.build({
	...
	handler: async ({ input, ctx }: { input: z.infer<typeof createWalletMonitoringInputSchema>; ctx: AuthContext }) => {
		...
		if (existing) {
			throw createHttpError(409, 'Monitoring config already exists for this payment source. Use PATCH to update it.');
		}
		...
		if (hotWallets.length !== hotWalletIds.length) {
			throw createHttpError(400, 'One or more hot wallets not found or do not belong to this payment source');
		}
...
export const updateWalletMonitoringInputSchema = z.object({
	...
	checkIntervalSeconds: z.coerce.number().min(60).max(86400).optional().describe('Update check interval in seconds'),
});
...
export const walletMonitoringPatch = adminAuthenticatedEndpointFactory.build({
	...
	output: walletMonitorConfigSchema,
	handler: async ({ input, ctx }: { input: z.infer<typeof updateWalletMonitoringInputSchema>; ctx: AuthContext }) => {
...
export const walletMonitoringDelete = adminAuthenticatedEndpointFactory.build({
	...
	output: walletMonitorConfigSchema,
	handler: async ({ input, ctx }: { input: z.infer<typeof deleteWalletMonitoringInputSchema>; ctx: AuthContext }) => {
		const existing = await prisma.walletMonitorConfig.findFirst({
			where: {
				id: input.id,
				PaymentSource: {
