...
export async function checkAllWalletBalances(): Promise<void> {
	try {
		...
		const results = await Promise.allSettled(dueConfigs.map((config) => checkPaymentSourceWallets(config)));
...
async function checkPaymentSourceWallets(config: ConfigWithRelations): Promise<void> {
	...
	try {
		...
		const results = await Promise.allSettled(
			WalletThresholds.map((threshold) => checkWalletBalance(apiKey, PaymentSource.network, threshold)),
		);
...
async function checkWalletBalance(
	...
	try {
		...
		if (AssetThresholds.length > 0 && result.assetBalances) {
			checkAssetThresholds(AssetThresholds, result.assetBalances, HotWallet, address, network);
		}
	} catch (error) {
...
async function fetchWalletBalances(apiKey: string, address: string): Promise<WalletCheckResult> {
	const maxRetries = 3;
...
function checkAssetThresholds(
	...
	for (const threshold of assetThresholds) {
		...
		updateAssetBalance(address, network, threshold.policyId, threshold.assetName, currentBalance);

		if (currentBalance < threshold.minAmount) {
			const safeDecimals = Math.min(threshold.decimals, 18);

			const displayBalance = Number(currentBalance) / Math.pow(10, safeDecimals);
			const displayThreshold = Number(threshold.minAmount) / Math.pow(10, safeDecimals);
