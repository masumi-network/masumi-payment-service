...
import { getLovelaceFromUtxo, selectCollateralUtxo, sortAndLimitUtxos, sortUtxosByLovelaceDesc } from './index';
...
describe('UTxO selection', () => {
	...
	it('does not use a token-bearing UTxO as collateral', () => {
		const tokenUtxo = createUtxo('token', 6_000_000n, [{ unit: 'policy.asset', quantity: '1' }]);

		expect(() => selectCollateralUtxo([createUtxo('small', 4_999_999n), tokenUtxo])).toThrow(
			'Pure-ADA collateral UTxO not found',
		);
	});
