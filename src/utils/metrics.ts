...
const getBlockchainStateTransitionCounter = (): Counter => {
	if (!blockchainStateTransitionCounter) {
		blockchainStateTransitionCounter = getMeter().createCounter('blockchain_state_transitions_total', {
...
const extractBusinessErrorDetails = (error: Error | string, endpoint: BusinessEndpoint) => {
	...
	if (lowerMessage.includes('unauthorized')) return { error_code: 'UNAUTHORIZED', error_category: 'auth' };
	if (lowerMessage.includes('forbidden')) return { error_code: 'FORBIDDEN', error_category: 'auth' };
	if (lowerMessage.includes('not found')) return { error_code: 'NOT_FOUND', error_category: 'business_logic' };
