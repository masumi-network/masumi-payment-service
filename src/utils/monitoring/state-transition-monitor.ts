		...
		try {
			...
			while (hasMore) {
				...
				for (const purchase of purchases) {
					...
					if (!StateTransitionMonitor.MONITORED_PURCHASE_ACTIONS.has(purchase.NextAction.requestedAction)) {
						skippedCount++;
						continue;
					}
			...
			logger.info(`Processed ${processedCount} purchase state changes (skipped ${skippedCount} non-monitored)`);
		} catch (error) {
		...
		try {
			...
			while (hasMore) {
				...
				for (const payment of payments) {
					...
					if (!StateTransitionMonitor.MONITORED_PAYMENT_ACTIONS.has(payment.NextAction.requestedAction)) {
						skippedCount++;
						continue;
					}
			...
			logger.info(`Processed ${processedCount} payment state changes (skipped ${skippedCount} non-monitored)`);
		} catch (error) {
