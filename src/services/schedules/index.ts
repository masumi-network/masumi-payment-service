...
export async function initJobs() {
	...
	void new Promise((resolve) => setTimeout(resolve, 750)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info('Finished to check for batched payments in ' + (new Date().getTime() - start.getTime()) / 1000 + 's');
	...
	void new Promise((resolve) => setTimeout(resolve, 5000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info(
				'Finished to check for outstanding payments in ' + (new Date().getTime() - start.getTime()) / 1000 + 's',
			);
	...
	void new Promise((resolve) => setTimeout(resolve, 10000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info('Finished to check for refunds in ' + (new Date().getTime() - start.getTime()) / 1000 + 's');
	...
	void new Promise((resolve) => setTimeout(resolve, 15000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info(
				'Finished to check for timed out refunds in ' + (new Date().getTime() - start.getTime()) / 1000 + 's',
			);
	...
	void new Promise((resolve) => setTimeout(resolve, 23000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info('Finished to check to authorize refunds in ' + (new Date().getTime() - start.getTime()) / 1000 + 's');
	...
	void new Promise((resolve) => setTimeout(resolve, 25000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info(
				'Finished to check for agent registration in ' + (new Date().getTime() - start.getTime()) / 1000 + 's',
			);
	...
	void new Promise((resolve) => setTimeout(resolve, 30000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info(
				'Finished to check for agent deregistration in ' + (new Date().getTime() - start.getTime()) / 1000 + 's',
			);
	...
	void new Promise((resolve) => setTimeout(resolve, 40000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			await checkLatestTransactions();
			logger.info(
				'Finished to sync cardano payment entries in ' + (new Date().getTime() - start.getTime()) / 1000 + 's',
			);
	...
	void new Promise((resolve) => setTimeout(resolve, 45000)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info('Finished to check for submit result in ' + (new Date().getTime() - start.getTime()) / 1000 + 's');
	...
	void new Promise((resolve) => setTimeout(resolve, 7500)).then(() => {
		...
		AsyncInterval.start(async () => {
			...
			logger.info('Finished automatic decision handler in ' + (new Date().getTime() - start.getTime()) / 1000 + 's');
	...
	void new Promise((resolve) => setTimeout(resolve, 50000)).then(() => {
		...
		AsyncInterval.start(
			async () => {
				...
				logger.info('Finished webhook cleanup in ' + (new Date().getTime() - start.getTime()) / 1000 + 's');
			},
			24 * 60 * 60 * 1000,
		);
	});
