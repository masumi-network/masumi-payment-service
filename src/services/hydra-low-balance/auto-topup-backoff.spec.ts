...
describe('evaluateAutoTopupBackoff', () => {
	...
	it('holds a rule whose last attempt failed a minute ago', () => {
		const backoff = evaluateAutoTopupBackoff([{ status: HydraTopupStatus.Failed, updatedAt: ago(60_000) }], NOW);
	...
	it('releases it once the wait has passed', () => {
		const backoff = evaluateAutoTopupBackoff(
			[{ status: HydraTopupStatus.Failed, updatedAt: ago(AUTO_TOPUP_BACKOFF_BASE_MS + 1) }],
			NOW,
		);
	...
	it('waits longer the more attempts in a row have failed', () => {
		const attempts = [
			{ status: HydraTopupStatus.Failed, updatedAt: ago(11 * 60_000) },
			{ status: HydraTopupStatus.Failed, updatedAt: ago(20 * 60_000) },
			{ status: HydraTopupStatus.Failed, updatedAt: ago(30 * 60_000) },
		];
	...
	it('does not count failures behind a settled attempt', () => {
		const backoff = evaluateAutoTopupBackoff(
			[
				{ status: HydraTopupStatus.Absorbed, updatedAt: ago(10_000) },
				{ status: HydraTopupStatus.Failed, updatedAt: ago(60_000) },
				{ status: HydraTopupStatus.Failed, updatedAt: ago(120_000) },
			],
			NOW,
		);
	...
	it('holds rather than retries when the failure has no usable timestamp', () => {
		const backoff = evaluateAutoTopupBackoff([{ status: HydraTopupStatus.Failed, updatedAt: new Date(NaN) }], NOW);
