...
describe('withJobLock', () => {
	...
	it('skips an overlapping cycle with a job-specific message', async () => {
		...
		await expect(withJobLock(mutex, 'submit_result_v1', operation)).resolves.toBeUndefined();

		expect(operation).not.toHaveBeenCalled();
		expect(mockInfo).toHaveBeenCalledWith('submit_result_v1 is already running, skipping cycle');
		release();
	});
