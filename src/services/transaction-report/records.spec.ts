...
describe('buildReportRow', () => {
	it('reconciles one legacy seller relation with only the seller fee component', () => {
	...
	it('marks actor fees partial for requests created before actor fee columns existed', () => {
		const row = buildReportRow(
			record({ hasCompleteActorCardanoFeeHistory: false }),
			'Billable',
		...
		expect(row.cardanoFeeReconciliation.completeness).toBe('partial');
		expect(getReportRowWarnings(row).map((warning) => warning.code)).toContain('LEGACY_ACTOR_CARDANO_FEES_PARTIAL');
	});
	...
	it.each(['CreatedAt', 'FundsLockedAt'] as const)('keeps lifetime buyer values for %s cohorts', (dateBasis) => {
		...
		expect(row.actorCardanoFeeAllocation).toMatchObject({ strategy: 'lifetime_cohort', completeness: 'complete' });
	});
...
describe('serializeReportRow', () => {
	...
	it('keeps BigInt values as exact strings at the JSON boundary', () => {
		...
		expect(serialized.cardanoFeeReconciliation).toMatchObject({
			...
			adminCardanoFees: expect.objectContaining({ rawAmount: '100000', symbol: 'ADA' }),
