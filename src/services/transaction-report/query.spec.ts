...
describe('queryReportPage', () => {
	...
	it('marks pre-version actor fee accounting as historical partial evidence', async () => {
		mockFindPayments.mockResolvedValue([
			{ ...paymentRecord('legacy-fees', '2026-01-03T00:00:00.000Z'), cardanoFeeAccountingVersion: 0 },
		]);

		const result = await queryReportPage(filters({ roles: ['Seller'] }), { Buyer: null, Seller: null }, 50);

		expect(result.records[0].hasCompleteActorCardanoFeeHistory).toBe(false);
	});
