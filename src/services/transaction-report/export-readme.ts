...
export function createReportReadme(metadata: ReportCsvMetadata): Buffer {
	...
	const lines = [
		...
		'',
		'Every added-up figure also carries a `*_completeness` column. `complete` means the figure is exact.',
		'`partial` next to a number means the number is a floor or an apportioned estimate, so treat it as the',
		'least the figure can be. `partial` next to an empty cell means the figure is unknown.',
		'',
