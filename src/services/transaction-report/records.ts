...
export type ReportRequestRecord = {
	...
	hasCompleteActorCardanoFeeHistory: boolean;
...
export function buildReportRow(
	...
	const actorCardanoFeeAllocation = {
		...
		completeness:
			!record.hasCompleteActorCardanoFeeHistory ||
			!hasKnownFundsLockedMembership ||
			(window.dateBasis === 'RevenueRecognizedAt' && (record.sellerCardanoFees > 0n || record.buyerCardanoFees > 0n))
				? ('partial' as const)
				: ('complete' as const),
		attachedAt:
	...
	const cardanoFeeReconciliation = reconcileRowCardanoFees(
		...
		actorCardanoFeeAllocation.completeness === 'complete',
		feeComponentScope,
	);
...
export function getReportRowWarnings(row: ReportRow): ReportWarning[] {
	...
	if (row.actorCardanoFeeAllocation.completeness === 'partial') {
		warnings.push({
			...
			message:
				'Stored actor Cardano fees are cumulative. Revenue-recognized reports attach them to one accounting event because their exact transaction time is unavailable.',
			rowId: row.id,
		});
	}
	if (!row.hasCompleteActorCardanoFeeHistory) {
		warnings.push({
			code: 'LEGACY_ACTOR_CARDANO_FEES_PARTIAL',
			message:
				'This request predates versioned actor-fee accounting and has no verified backfill. Buyer, seller, and admin fee allocation can be incomplete.',
			rowId: row.id,
		});
	}
