...
export async function updateWalletTransactionHash() {
	...
	try {
		...
		// Handle wallets with pending fund transfers
		const fundTransferLockedWallets = await prisma.hotWallet.findMany({
			where: {
				PendingFundTransfer: {
					OR: [{ lastCheckedAt: { lte: new Date(Date.now() - 1000 * 60 * 1) } }, { lastCheckedAt: null }],
				},
		...
		await Promise.allSettled(
			fundTransferLockedWallets.map(async (wallet) => {
				try {
					...
					try {
						...
						logger.error(`Failed to update fund transfer ${fundTransferId}: ${errorToString(fundTxError)}`);
					}
					...
					logger.error(`Error updating fund transfer wallet transaction hash: ${errorToString(error)}`);
				}
			}),
		);
