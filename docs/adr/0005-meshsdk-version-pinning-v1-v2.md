...
| `packages/payment-source-v2`             | `1.9.0-beta.103`     | `1.9.1` (stable)      | V2 services          |
...
- Files that do the same for **V2** payment sources import Mesh exclusively
  ...
  (`@meshsdk/core@1.9.0-beta.103`, `@meshsdk/core-cst@1.9.1`).

  The V2 `core-cst` moved to stable `1.9.1` after verifying the derived
  V2 payment address is byte-identical across `beta.102`, `beta.103`,
  `1.9.0` and `1.9.1` (the V1 line, by contrast, derives a DIFFERENT
  address under anything newer than its own pins — V1 can never move).
  Tx serialization is unaffected by this: `MeshTxBuilder` comes from
  `@meshsdk/core@beta.103`, which nests its own `core-cst@beta.103` +
  `common@beta.103`, so the script-data-hash path and the runtime
  cost-model patching (`mesh-cost-model-sync`) still run entirely on the
  beta.103 instance graph. The direct `core-cst@1.9.1` imports are pure
  helpers (param application, address/key resolution, datum/value
  conversion).
