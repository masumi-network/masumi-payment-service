...
6. **Smoke-test V1 and V2 flows.** Register a new V1 agent and confirm legacy-shaped list/count/diff calls still return only V1 rows. Register a new V2 agent. Open a V2 purchase via the new `smartContractAddress` field. Verify the inbox routes work for both V1 and V2 sources when the relevant `filterSmartContractAddress` is supplied.
...
The public route namespace stays `/api/v1`. To keep pre-V2 clients compatible,
legacy-shaped list/count/diff and aggregate calls default to `Web3CardanoV1`
when no V2-aware selector is supplied. This applies to regular registry,
inbox registry, payment, purchase, payment income, and purchase spending
queries.

New clients opt into V2 or mixed-source behavior by passing
`filterSmartContractAddress` (exact source address) or `filterPaymentSourceType`
where that parameter exists. Registry payment-target discovery additionally
uses `filterSupportedPaymentSourceAddress` or the comma-separated
`filterSupportedPaymentSourceNetworks` query string; those filters also opt out
of the default V1 compatibility filter.
