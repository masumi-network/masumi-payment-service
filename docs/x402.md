...

{
  ...
  "paymentRequirements": { /* the exact 402 requirements your server issued */ }
}
```
...
authoritative. For dynamic pricing it is required and must be supplied by the
API key that owns the registry entry (or an admin); the buyer-controlled
`paymentPayload.accepted` is never used to choose the amount. Settlement checks
that the signed payload matches those trusted requirements on every
economically relevant field (network, asset, amount, payTo, timeout, transfer
method, decimals).

The current EVM `exact` integration settles ERC-20 tokens through Permit2. Chain
native currencies are used for facilitator gas and balance monitoring, but are
not advertised as payable x402 assets until a compatible native settlement
