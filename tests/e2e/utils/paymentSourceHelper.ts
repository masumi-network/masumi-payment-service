 ...
 */
export async function getActiveSmartContractAddress(network: Network, apiClient?: ApiClient): Promise<string> {
	...
	if (!paymentSource) {
		throw new Error(`No active PaymentSource found for network ${network}. Please run database seeding first.`);
	}
...
export async function getActiveWalletVKey(
	...
	if (wallets.length === 0) {
		throw new Error(`No active ${walletType} wallet found for network ${network}. Please run database seeding first.`);
	}
