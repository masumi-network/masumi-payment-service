			...
			() =>
				prisma.$transaction(
					async (prisma) => {
						...
						// note in V2's handleV2PaymentTransaction).
			...
			() =>
				prisma.$transaction(
					async (prisma) => {
						...
						// Read currentAction FRESH inside this Serializable tx (see V2's
						// handleV2PaymentTransaction for the race-window rationale).
