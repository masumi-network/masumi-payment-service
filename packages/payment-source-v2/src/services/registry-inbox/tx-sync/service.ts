...
export async function checkInboxAgentRegistrationTransactionsV2() {
	await withJobLock(mutex, 'registry_inbox_tx_sync_v2', async () => {
		try {
			const paymentContracts = await getPaymentSourcesForSync();
			if (paymentContracts.length === 0) {
				logger.warn('No V2 payment contracts found for inbox tx sync, skipping update');
				return;
			}
			...
			try {
				...
				if (failedResults.length > 0) {
					logger.error('Error updating V2 inbox agent registration requests', {
						error: failedResults,
						paymentContracts,
					});
				}
			} catch (error) {
				logger.error('Error checking latest V2 inbox agent registration transactions', { error });
			}
		} catch (error) {
			logger.error('Error checking latest V2 inbox agent registration transactions', { error });
		}
...
async function syncInboxAgentRegistrationRequests(
	...
	const results = await advancedRetryAll({
		operations: registrationRequests.map((request) => async () => {
			const owner = await blockfrost.assetsAddresses(request.agentIdentifier!, { order: 'desc' });
			...
			if (request.state === RegistrationState.RegistrationInitiated) {
				if (owner.length >= 1 && owner[0].quantity === '1') {
					if (request.CurrentTransaction == null || request.CurrentTransaction.txHash == null) {
						throw new Error('V2 inbox agent registration request has no tx hash');
					}
					...
					if (request.CurrentTransaction.BlocksWallet != null) {
						await prisma.hotWallet.update({
							where: { id: request.CurrentTransaction.BlocksWallet.id, deletedAt: null },
							data: { lockedAt: null },
						});
					}
				} else {
				...
				if (owner.length === 0 || owner[0].quantity === '0') {
					if (request.CurrentTransaction == null || request.CurrentTransaction.txHash == null) {
						throw new Error('V2 inbox deregistration request has no tx hash');
					}
					const tx = await blockfrost.txs(request.CurrentTransaction.txHash);
					const block = await blockfrost.blocks(tx.block);
					await prisma.inboxAgentRegistrationRequest.update({
		...
		errorResolvers: [
			delayErrorResolver({
				configuration: { maxRetries: 5, backoffMultiplier: 2, initialDelayMs: 500, maxDelayMs: 1500 },
			}),
		],
	});
	...
	results.forEach((result) => {
		if (result.success === false) {
			logger.warn('Failed to update V2 inbox agent registration request', { error: result.error });
		}
	});
}
...
async function getInboxAgentRegistrationRequestsToSync(paymentContractId: string) {
	return await prisma.inboxAgentRegistrationRequest.findMany({
		where: {
			PaymentSource: { id: paymentContractId },
			...
			updatedAt: { lt: new Date(Date.now() - 1000 * 60 * 1) },
		},
		include: { CurrentTransaction: { include: { BlocksWallet: true } } },
	});
}
