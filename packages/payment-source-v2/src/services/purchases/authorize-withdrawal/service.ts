...
const markRequestFailed = makePurchaseRequestFailureMarker({
	logMessage: 'Error authorizing V2 withdrawal',
	errorNotePrefix: 'Authorizing V2 withdrawal failed: ',
});

const unlockHotWallet = makeHotWalletUnlocker('authorize-withdrawal');
...
async function processSinglePurchaseRequest(
	...
	if (newTxHash !== intendedTxHash) {
		logger.error('V2 authorize-withdrawal single-item: node returned divergent txHash — investigate', {
			intendedTxHash,
...
async function processWalletBatch(
	...
	for (const request of requests) {
		try {
			...
			if (isLookupDeferred(error)) {
				...
				logger.warn('V2 authorize-withdrawal: deferring item this tick (chain lookup not ready) [batch-diag]', {
					tickPhase: 'validate',
				...
				logger.warn('V2 authorize-withdrawal: marking item as failed (non-defer error) [batch-diag]', {
					requestId: request.id,
					blockchainIdentifier: request.blockchainIdentifier,
	...
	logger.warn('V2 authorize-withdrawal: per-item validation outcome [batch-diag]', {
	...
	if (validated.length === 0) {
		logger.info(
			'No V2 authorize-withdrawal items in this tick reached the batch builder (every item was either deferred for tx-sync to reconcile or marked failed); leaving wallet unlocked',
	...
	try {
		sharedTxId = await retryOnSerializationConflict(
			...
			{ label: 'authorize-withdrawal-batch-tx' },
		);
	} catch (dbError) {
	...
	try {
		await retryOnSerializationConflict(
			...
			{ label: 'authorize-withdrawal-batch-record-intended' },
		);
	} catch (recordError) {
		logger.error('V2 authorize-withdrawal batch could not record intendedTxHash; will rollback pre-submit DB', {
			sharedTxId,
			intendedTxHash,
	...
	try {
		...
		if (!definitive) {
			logger.warn('V2 authorize-withdrawal batch submit AMBIGUOUS; leaving Pending for reconciliation', {
				sharedTxId,
				intendedTxHash,
		...
		logger.warn(
			'V2 authorize-withdrawal batch submit definitively rejected; rolling back DB and retrying as single items',
			{
				error:
					submitError instanceof Error
						? { message: submitError.message, stack: submitError.stack, name: submitError.name }
	...
	logger.warn('V2 authorize-withdrawal: batch submitted [batch-diag]', {
		tickPhase: 'submitted',
		walletId: wallet.id,
		newTxHash,
		sharedTxId,
...
async function processL2AuthorizeWithdrawal(
	...
	const outcome = await submitReservedL2Action({
		...
		operation: 'authorize-withdrawal',
		requestId: request.id,
		nextActionId: request.nextActionId,
		previousTransactionId: request.CurrentTransaction!.id,
		walletId: purchasingWallet.id,
		walletLockedAt: purchasingWallet.lockedAt!,
		hydraHeadId: headId,
		signedTx,
		initiatedAction: PurchasingAction.AuthorizeWithdrawalInitiated,
		retryAction: PurchasingAction.AuthorizeWithdrawalRequested,
