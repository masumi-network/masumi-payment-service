...
async function processSingleRefundCollection(
	...
	if (newTxHash !== intendedTxHash) {
		logger.error('V2 collect-refund single-item: node returned divergent txHash — investigate', {
			intendedTxHash,
...
async function processWalletBatch(
	...
	if (newTxHash !== intendedTxHash) {
		logger.error('V2 collect-refund batch: node returned divergent txHash — investigate', {
			sharedTxId,
			intendedTxHash,
			nodeTxHash: newTxHash,
			requestIds: fit.map((v) => v.request.id),
		});
		recordV2BatchHashDivergence('collect-refund', { source_id: paymentContract.id });
	}
...
/** The L2 portion of the CollectRefund cycle, shared by the timer and the on-demand run. */
async function runCollectRefundL2Pass(): Promise<void> {
		...
		l2PaymentContracts.map(async (paymentContract) => {
			...
			await Promise.allSettled(
				paymentContract.PurchaseRequests.map(async (request) => {
					try {
						const progressed = await processL2CollectRefund(request, paymentContract, network);
						if (!progressed) {
 ...
 */
export async function collectRefundL2V2(): Promise<void> {
	let release: MutexInterface.Releaser | null;
	try {
		release = await tryAcquire(mutex).acquire();
	} catch {
		logger.info('collect_refund_v2 is already running, skipping the on-demand L2 pass');
		return;
	}

	try {
		await runCollectRefundL2Pass();
	} catch (error) {
