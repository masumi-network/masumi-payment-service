...
/**
 * Get agent earnings analytics. (READ access required)
 * Get agent earnings and fee analytics over specified time periods.
 */
export const getRegistryEarnings = <ThrowOnError extends boolean = false>(options: Options<GetRegistryEarningsData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetRegistryEarningsResponse, unknown, ThrowOnError>({
        responseType: 'json',
        security: [
            {
                name: 'token',
