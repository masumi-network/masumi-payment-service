...
They never touch L1 — that is what a Hydra head is for. Cardano L1 tops out near 10-15 TPS, so
...
gunzip -c bench/saturation/events.ndjson.gz | jq -s '[.[]|select(.kind=="confirmed")]|length'
```
