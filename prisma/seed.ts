		...
		if (shouldSkipV1) {
			...
			try {
				...
				if (
					...
					try {
						// dev seeds the preprod PaymentSource inside an anonymous $transaction,
						// so look it up by the contract address we just seeded.
						const paymentSourcePreprod = await prisma.paymentSource.findFirstOrThrow({
							where: { smartContractAddress: smartContractAddress, network: Network.Preprod },
						});
