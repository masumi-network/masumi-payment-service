...
-- wallets are EXACTLY the 3 seed DEFAULTS (all present, none extra). A source's
-- admin wallets derive its payment address, and an exact-default admin set already
-- matches the default address, so we only move the address + policyId — AdminWallet
-- rows are left untouched (avoiding the onDelete: Restrict FK and the deferrable
-- count(AdminWallets) >= requiredAdminSignatures trigger). Genuinely-CUSTOM sources
-- (any admin != a default, or a subset of defaults with a different derived
-- address) are intentionally NOT migrated here — they are surfaced by
-- warnOutOfSyncV2PaymentSources / the frontend "Outdated contract" badge for manual
-- handling (their correct new address derives from their own admin wallets and
-- cannot be a hardcoded default).
--
-- SAFETY: guarded to at most ONE such source per network, and only when the default
-- identity is not already occupied — the address is unique across ALL rows
-- (@@unique([network, smartContractAddress]), NOT partial) and the policyId is
-- unique across ACTIVE rows (PaymentSource_network_policyId_active_key). Either
-- collision, or >1 stale-default source, is skipped (RAISE NOTICE) rather than
-- aborting the migration. NOTE: this runs unconditionally on every
-- `migrate deploy`; on-chain funds locked at the OLD address are NOT moved by this
-- (they need the OLD validator) — drain/settle them first per
