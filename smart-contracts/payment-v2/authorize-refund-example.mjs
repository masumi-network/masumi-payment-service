...
console.log('Authorizing refund as V2 payment example');
...
fields[17] = 0n;
fields[18] = stateData(State.RefundAuthorized);

const tx = new Transaction({ initiator: wallet, fetcher: blockchainProvider })
	.redeemValue({
		value: utxo,
		script,
		redeemer: actionData(Action.AuthorizeRefund),
	})
...
console.log(`Created V2 authorize refund transaction:
    Tx ID: ${txHash}
